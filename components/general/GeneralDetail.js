...
export default function GeneralDetail(props) {
  ...
  const view = [];
  let headerLevel = props.lastHeaderLevel
    ? props.lastHeaderLevel + 1
    : initHeaderLevel;
  ...
  view.push(
    <header className={"header" + lastHeaderLevel} id={field.label}>
      {field.label}{" "}
      <a target="_blank" rel="noreferrer" href={WIKIBASE_URL + field.id}>
        &#x270E;
      </a>
    </header>
  );
  ...
  if (field.statements.elementof?.occurrences[0].id === "Q264") {
    //RDA-Eigenschaft
    let filter_sorted_statements = Object.entries(sorted_statements).filter(
      ([key, value]) =>
        (value.id === "P1") |
        (value.id === "P126") |
        (value.id === "P402") |
  ...
  } else if (field.statements.elementof?.occurrences[0].id === "Q2") {
    //GND-Datenfeld
    let filter_sorted_statements = Object.entries(sorted_statements).filter(
      ([key, value]) =>
        (value.id === "P1") |
  ...
  // <hr/>
  // )
  for (const [, statement] of Object.entries(sorted_statements)) {
    if (
      statement.id === "P1" ||
      statement.id === "P12" ||
      statement.id === "P110" ||
      ...
      //ignore definition,repetition,schema,element of
      continue;
    }
    if (statement.id === "P15") {
      // GND Subfields
      ...
      view.push(
        <header className={"header" + lastHeaderLevel} id={statement.label}>
          {statement.label}
        </header>
      );
    ...
    if (statement.id === "P397" || statement.id === "P398") {
      // embedded in (Property)/(Item)
      ...
      statement.occurrences.map((occ, index) => {
        ...
        if (occ.qualifiers) {
          if (
            Object.keys(occ.qualifiers).find((el) => el === "typeoflayout") ===
            undefined
          ) {
            view.push(<p key={index}>{occ.value}</p>);
          }
          for (const [key, value] of Object.entries(occ.qualifiers)) {
            ...
            if (value.id === "P389") {
              ...
              switch (expr) {
                ...
                case "Q1343": // Zwischenueberschrift erster Ordnung
                  ...
                  view.push(
                    <header
                      className={"header" + lastHeaderLevel}
                      id={occ.value}
                    >
                      {occ.value}
                    </header>
            ...
            if (value.id === "P396" || value.id === "P411") {
              // embedded Item / embedded Property
            ...
            } else if (value.id === "P392" || value.id === "P393") {
              // see(item/property)
            ...
            } else if (value.id !== "P389" && value.id !== "P404") {
              // no Layouttyp /(embedded element)
