...
export async function getRdaProperties() {
  ...
  bindings.map((binding) => {
    var label = binding["assignmentLabel"]
...
export async function readLabelEn(label) {
  const obj = {};
  label.map((item) => {
    var value_strip = item["elementLabel"].value
      .toLowerCase()
      .split(" ")
      .join("");
    obj[item["eId"].value] = {};
...
async function readLabelDe(label) {
  const obj = {};
  label.map((item) => {
    var label = item["elementLabel"].value;
    var strip = label.indexOf(" - ");
    var strip2 = label.indexOf(" – ");
    if (strip > 0) {
      label = label.substring(strip + 3);
...
async function readCodings(codings) {
  const obj = {};
  codings.map((coding) => {
    var key = coding["eId"].value;
    var key_filter = codings.filter((coding) => coding["eId"].value === key);
    obj[key] = {};
...
async function recursiveRenderEntity(id, headerLevel) {
  ...
  var label = entity.labels?.de?.value ?? "Kein Label definiert";
