...
export async function getElements( sparqlQuery ) {
  ...
  bindings.map( binding => {
    ...
    obj[binding['eId'].value]['assignmentId'] = binding['assignmentId'] ? binding['assignmentId'].value : ''
    obj[binding['eId'].value]['assignmentLabel'] = binding['assignmentLabel'] ? binding['assignmentLabel'].value : ''
  })
  return obj
}
...
async function getLink(assignmentId, elementId) {
  // let assignmentId = await lookup.assignmentId
  let link = ''
  switch (assignmentId) {
    case 'Q3095': // Regeltext
      link = '/rda/rules/' + elementId
      break
    case 'Q264': // RDA-Eigenschaft
      link = '/rda/properties/' + elementId
      break
    case 'Q2': // GND-Datenfeld
      link = '/gnd/fields/' + elementId
      break
    case 'Q3': // GND-Unterfeld
      link = '/gnd/subfields/' + elementId
      break
    case 'Q4': // GND-Relationstyp
      link = '/gnd/relationtypes/' + elementId
      break
    default:
      link = '/general/' + elementId
      break
  }
  return link
}
...
let entityCounter = 1
async function recursiveRenderEntity(lookup_en, lookup_de, codings, id, entityCounter) {
  const wikiurl = API_URL + '/w/api.php?action=wbgetentities&format=json&languages=de&ids=' + id
  ...
  // second level
  obj['statements'] = {}
  await Promise.all(Object.keys(entity.claims).map(async (key) => {
    ...
    if (key === 'P4') { // integrate coding from codings const
      obj['statements'][lookup_en[key].label]['format'] = {}
      obj['statements'][lookup_en[key].label]['format'] = codings[id]['coding']['format']
    }
    if (codings[key] !== undefined) {
      obj['statements'][lookup_en[key].label]['coding'] = codings[key]['coding']
    }
    obj['statements'][lookup_en[key].label]['occurrences'] = []
    // third level
    await Promise.all(entity.claims[key].map(async (occurrence,index) => {
      if (occurrence.mainsnak.snaktype === 'value' && occurrence['mainsnak']['datavalue']['value']['id'] !== undefined) {
        let occurrence_id = occurrence['mainsnak']['datavalue']['value']['id']
        const statement_link = await getLink(lookup_en[occurrence_id].assignmentId,occurrence_id)
        ...
        obj['statements'][lookup_en[key].label]['occurrences'][index] = {'value':occurrence['mainsnak']['datavalue']['value']}
      } else {
        obj['statements'][lookup_en[key].label]['occurrences'][index] = {'value':''}
      }
      if (key === 'P15') { // integrate coding in subfields (P15) object
        let occurrence_id = occurrence['mainsnak']['datavalue']['value']['id']
        obj['statements'][lookup_en[key].label]['occurrences'][index]['coding'] = codings[occurrence_id]['coding']
      }
      if (key === 'P11' || key === 'P396' || key === 'P411') { // integrate example (P11) OR embedded item (P396) OR embedded property (P411)
        let occurrence_id = occurrence['mainsnak']['datavalue']['value']['id']
        // obj['statements'][lookup_en[key].label]['occurrences'][index]['statements'] = examples[occurrence_id]['statements']
        obj['statements'][lookup_en[key].label]['occurrences'][index] = await recursiveRenderEntity(lookup_en, lookup_de, codings, occurrence_id, entityCounter++)
      }
      // fourth level
      if (occurrence['qualifiers'] !== undefined) {
        obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'] = {}
        await Promise.all(Object.keys(occurrence['qualifiers']).map(async (quali_key,quali_index) => {
          ...
          if (codings[quali_key]) {
            obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['coding'] = codings[quali_key]['coding']
          }
          // fifth level
          await Promise.all(occurrence['qualifiers'][quali_key].map(async (occurrences2,index2) => {
            let occurrences2_id = undefined
            if(occurrences2['datavalue'] !== undefined) {
              occurrences2_id = occurrences2['datavalue']['value']['id']
              if (occurrences2_id) {
                const qualifier_link = await getLink(lookup_en[occurrences2_id].assignmentId,occurrences2_id)
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = {}
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['id'] = occurrences2_id
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['label'] = lookup_de[occurrences2_id]
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['link'] = qualifier_link
                if(codings[occurrences2_id]) {
                  obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['coding'] = codings[occurrences2_id]['coding']
                }
                if (quali_key === 'P11' || quali_key === 'P396' || quali_key === 'P411') { // integrate example (P11) OR embedded item (P396) OR embedded property (P411)
                  ...
                  obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = await recursiveRenderEntity(lookup_en, lookup_de, codings, occurrences2_id, entityCounter++)
                }
              } else if (occurrences2['datatype'] === 'time') {
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = {'value':occurrences2['datavalue']['value']['time']} 
              } else {
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = {'value':occurrences2['datavalue']['value']} 
              }
            }
          }))
        }))
      ...
      if (occurrence['references'] !== undefined) {
        obj['statements'][lookup_en[key].label]['occurrences'][index]['references'] = []
        occurrence['references'].map((reference,ref_index) => {
          ...
          obj['statements'][lookup_en[key].label]['occurrences'][index]['references'][ref_index] = ref_obj
        })
      }
    }))
  }))
...
export async function getEntity(entityId) {
  ...
  const lookup_de = await getLabels( sparql.LABELDE )
  const codings = await getCodings( sparql.CODINGS )
  const entity = await recursiveRenderEntity(lookup_en, lookup_de, codings, entityId, entityCounter)
  return entity 
}
    ...
    claim.map((occ,index) => {
      ...
      if (occ['qualifiers']) {
        ...
        for (const [key4, qualifier] of Object.entries(occ.qualifiers)) {
          obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label] = {}
          obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['label'] = lookup_de[key4]
          obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['id'] = key4
          obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['occurrences'] = []
          // insert codings if possible (OBJ)
          if (codings[key4]) {
            obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['coding'] = codings[key4]['coding']
          }
          // fifth level (ARR)
          qualifier.map((quali_value,quali_index) => {
            obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['occurrences'][quali_index] = {}
            if (quali_value['datatype'] === 'string') {
              obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['occurrences'][quali_index]['value'] = quali_value['datavalue']['value']
            }
            ...
            if (quali_value['datatype'] === 'wikibase-property' && quali_value['snaktype'] === 'value') {
              ...
              if ( codings[property] ) {
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['value'] = codings[property]['coding']
              }
            }
            ...
            if (key4 === 'P11') {
              obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[key4].label]['occurrences'][quali_index]['statements'] = examples[quali_value['datavalue']['value']['id']]['statements']
            }
          })
        }
      }
...
export async function getExample( exampleId ) {
  ...
  Object.keys(element.claims).map((claim_key) => {
    ...
    obj['statements'][lookup_en[claim_key].label]['label'] = lookup_de[claim_key]
    if (codings[claim_key] !== undefined) {
      obj['statements'][lookup_en[claim_key].label]['coding'] = codings[claim_key]['coding']
    }
    obj['statements'][lookup_en[claim_key].label]['occurrences'] = []
    // third level
    element.claims[claim_key].map((occurrence,index) => {
      if (occurrence.mainsnak.snaktype === 'value' && occurrence['mainsnak']['datavalue']['value']['id'] !== undefined) {
        obj['statements'][lookup_en[claim_key].label]['occurrences'][index] = {'id':occurrence['mainsnak']['datavalue']['value']['id'],'label':lookup_de[occurrence['mainsnak']['datavalue']['value']['id']]}
      } else if (occurrence['mainsnak']['snaktype'] === 'value' && occurrence['mainsnak']['datavalue']['value'] !== undefined) {
        obj['statements'][lookup_en[claim_key].label]['occurrences'][index] = {'value':occurrence['mainsnak']['datavalue']['value']}
      } else {
        obj['statements'][lookup_en[claim_key].label]['occurrences'][index] = {'value':''}
      }
      // fourth level
      if (occurrence['qualifiers'] !== undefined) {
        obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'] = {}
        const qualifiers_arr = Object.keys(occurrence['qualifiers'])
        qualifiers_arr.map(async (quali_key) => {
          obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label] = {}
          obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['label'] = lookup_de[quali_key]
          obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['id'] = quali_key
          // include coding block, if qualifier is property with coding
          if (codings[quali_key]) {
            obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['coding'] = codings[quali_key]['coding']
          }
          // fifth level
          let linkedItems = []
          ...
          if (linkedItems.length > 0) {
            ...
            obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'] = items
          }
          occurrence['qualifiers'][quali_key].map((quali_value,quali_index) => {
            if (quali_value['datatype'] === 'string') {
              obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['value'] = quali_value['datavalue']['value']
            }
            if (quali_value['datatype'] === 'wikibase-item' && quali_value['snaktype'] === 'value') {
              obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['value'] = { 
                id: quali_value['datavalue']['value']['id'], 
                label: lookup_de[quali_value['datavalue']['value']['id']]
              }
            }
            if (quali_value['datatype'] === 'wikibase-property' && quali_value['snaktype'] === 'value') {
              let property = quali_value['datavalue']['value']['id']
              if ( codings[property] ) {
                obj['statements'][lookup_en[claim_key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['value'] = codings[property]['coding']
              }
            }
            if (quali_value['datavalue'] && quali_value['datavalue']['type'] === 'wikibase-entityid') {
...
export function sortStatements( obj ) {
  ...
  var template = [
    ...
    'P385','P126','P402','P388','P386','P387','P410',
    // Eigenschaften für den Namensraum GND-Datenmodell
    'P14','P10','P15','P9','P60','P13','P16','P132','P329','P382','P383',
    // Datenfelder
    // Idents & Codes
...
export async function getField( fieldId ) {
  ...
  statements_arr.map((key) => {
    ...
    if (key === 'P4') { // integrate coding from codings api
      obj['statements'][lookup_en[key].label]['format'] = {}
      obj['statements'][lookup_en[key].label]['format'] = codings[fieldId]['coding']['format']
    }
    obj['statements'][lookup_en[key].label]['occurrences'] = []
    ...
    occurrences_arr.map((occurrences,index) => {
      ...
      if (key === 'P15') { // integrate coding in subfields (P15) object
        obj['statements'][lookup_en[key].label]['occurrences'][index]['coding'] = codings[occurrences_id]['coding']
      }
      if (key === 'P11') { // integrate examples (P11) object if available
        obj['statements'][lookup_en[key].label]['occurrences'][index]['statements'] = examples[occurrences_id]['statements']
      }
          ...
          occurrences_arr2.map((occurrences2,index2) => {
            ...
            if(occurrences2['datavalue'] !== undefined) {
              ...
              if (occurrences2_id) {
                ...
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['label'] = lookup_de[occurrences2_id]
                if (quali_key === 'P396') {
                  obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['statements'] = items[occurrences2_id]['statements']
                }
                if (quali_key === 'P11') {
                  obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2]['statements'] = examples[occurrences2_id]['statements']
                }
              } else {
                obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = {'value':occurrences2['datavalue']['value']} 
              }
