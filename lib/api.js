...
async function recursiveRenderEntity(lookup_en, lookup_de, codings, id) {
  ...
  await Promise.all(Object.keys(entity.claims).map(async (key) => {
    ...
    await Promise.all(entity.claims[key].map(async (occurrence,index) => {
      ...
      if (key === 'P11' || key === 'P396' || key === 'P411') { // integrate example (P11) OR embedded item (P396) OR embedded property (P411)
        ...
        obj['statements'][lookup_en[key].label]['occurrences'][index] = await recursiveRenderEntity(lookup_en, lookup_de, codings, occurrence_id)
      }
      // fourth level
            ...
            if(occurrences2['datavalue'] !== undefined) {
              ...
              if (occurrences2_id) {
                ...
                if (quali_key === 'P11' || quali_key === 'P396' || quali_key === 'P411') { // integrate example (P11) OR embedded item (P396) OR embedded property (P411)
                  ...
                  obj['statements'][lookup_en[key].label]['occurrences'][index]['qualifiers'][lookup_en[quali_key].label]['occurrences'][index2] = await recursiveRenderEntity(lookup_en, lookup_de, codings, occurrences2_id)
                }
...
export async function getEntity(entityId) {
  ...
  const lookup_de = await getLabels( sparql.LABELDE )
  const codings = await getCodings( sparql.CODINGS )
  const entity = await recursiveRenderEntity(lookup_en, lookup_de, codings, entityId)
  return entity 
}
